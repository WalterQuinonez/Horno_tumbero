...
from EquiposLAL import ArduinoUNO, OWONodp3063
import pyvisa 
...
class WorkerControl(QtCore.QThread):
    ...
    def __init__(self, pid_params=None):
        ...
        self.canal = 1
        self.max_voltage_source = 60

        # PID
        if pid_params is None:
            pid_params = dict(Kp=2.0, Ki=0.25, Kd=1.2)
        self.pid = PID(**pid_params)
        self.pid.output_limits = (0, self.max_voltage_source)
    ...
    def iniciar_conexiones(self):
        ...
        self.fuente  = OWONodp3063()
        #detecto en que COM# esta arduino}
        self.port_fuente = self.fuente.detectar_fuente_por_vidpid(self.rm)
        if self.port_fuente is None:
            raise RuntimeError("No se encontró fuente")
        # AHORA abro el puertp
        self.fuente_ser = self.fuente.conectar_fuente_OWON_visa(self.rm , self.port_fuente)
    ...
    def run(self):
        ...
        while not self._detener:
            ...
            self.fuente.set_voltaje(volt, self.canal)
